...
public class BluetoothModule implements Serializable {
	...
	private static final long serialVersionUID = 3721419759990402565L;
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
	    ...
	    public void onReceive(Context context, Intent intent) {
	        ...
	        if (BluetoothDevice.ACTION_FOUND.equals(action)) {
	            ...
	            ((MultiplayerMenuActivity)context).updateDetectedDevices(device.getName() + "&" + device.getAddress());
	        }
	    }
	};
	...
	private class ConnectThread extends Thread {
	    ...
	    public void run() {
	        ...
	        // Do work to manage the connection (in a separate thread)
	        Log.d("ConTh", String.format("CONNECTED # # # # # #"));
	        commsThread = new CommsThread(mmSocket);
	        commsThread.start();
	    }
	...
	public class CommsThread extends Thread{
	    ...
	    public void run() {
	        ...
	        while (true) {
	            try {
	                ...
	                mHandler.obtainMessage(MESSAGE_READ, bytes, -1, myString.substring(0,bytes)).sendToTarget();
	            } catch (IOException e) {
	                break;
	            }
	        }
	    ...
	    public void write(byte[] bytes) {
	        try {
	            ...
	            //Log.d("Written", "tamanho = " + bytes.length);
	        } catch (IOException e) { 
	...
	public boolean hasComms(){
		return commsThread != null ? true : false;
	}
